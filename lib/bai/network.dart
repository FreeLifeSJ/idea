...
class _NetworkAppPageState extends State<NetworkAppPage> {
  ...
  _decodebook(var body) {

    Map bookmap = json.decode(body);
    var model = BookModel.fromJson(bookmap);
  ...
  _decodeTest(var body) {
    ...
    storiesList.forEach((model) {
      print('${model.images}:${model.id}:${model.title}:${model.type}');
    });
